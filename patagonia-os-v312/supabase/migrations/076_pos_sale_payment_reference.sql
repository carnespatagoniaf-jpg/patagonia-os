-- Pedido real del dueño: si un cliente paga efectivo pero la cajera
-- aprieta "Tarjeta" por error (o al revés), la plata termina acreditada
-- en la cuenta de Tesorería que no es -- el arqueo de caja
-- (046_pos_shift_cash_reconciliation.sql) lo detecta al cerrar el turno,
-- pero recién ahí, ya con el error hecho. Para que la cajera no pueda
-- marcar un medio de pago que no sea efectivo sin tener el comprobante
-- real en la mano, ahora un pago que no sea en efectivo exige el número
-- de cupón/operación del posnet o transferencia -- crea fricción
-- exactamente donde se puede confundir, y de paso deja un dato para
-- cruzar contra el resumen del banco.
...
begin
  ...
  loop
    ...
    if v_payment_method <> 'cash' and v_payment_reference is null then
