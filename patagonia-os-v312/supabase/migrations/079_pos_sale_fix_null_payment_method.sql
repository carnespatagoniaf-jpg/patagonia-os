-- Bug real en producción: "Banco Provincia 2" existe y está activa, pero
-- su columna payment_method quedó en null desde que se creó (es una
-- columna nullable -- create_treasury_account no la exige). El chequeo
-- de "existe la cuenta" en create_pos_sale usaba
-- "if v_payment_method is null then raise 'Cuenta de tesorería inválida'"
-- -- eso confunde "no encontré la fila" con "encontré la fila pero el
-- método está vacío", así que rechazaba una cuenta real y activa. Se
-- usa `not found` (que plpgsql llena solo después del select into,
-- según haya devuelto fila o no) para el chequeo de existencia real.
