-- Bug real: el dueño desactivó una cuenta de tesorería (Banco Provincia 2)
-- mientras la cajera tenía Mostrador abierto en otra compu -- esa pestaña
-- siguió mostrando el botón de esa cuenta (la lista de cuentas activas no
-- se refresca sola), y al intentar cobrar ahí la venta se rechazaba sin
-- que el cajero (ni el dueño, mirando la pantalla) entendiera por qué --
-- create_pos_sale nunca chequeaba `active`, solo que la cuenta existiera,
-- así que el motivo real quedaba mudo. Ahora el rechazo es explícito.
