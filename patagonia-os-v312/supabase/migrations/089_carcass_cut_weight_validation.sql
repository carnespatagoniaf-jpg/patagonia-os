-- Mejora de auditoría: save_carcass_cut (030) no validaba que la suma de
-- los cortes no supere el peso total de la res -- no corrompía stock (cada
-- corte suma bien lo suyo a su producto), pero dejaba pasar un error de
-- tipeo sin avisar. Se agrega el chequeo: la suma de todos los cortes de
-- la res (sin contar el que se está editando, si corresponde) más el peso
-- nuevo no puede superar el total cargado para esa res.
...
begin
  ...
  select branch_id, total_weight into v_branch_id, v_total_weight
  ...
  select coalesce(sum(weight), 0) into v_other_cuts_weight
  from public.carcass_cuts
  where batch_id = p_batch_id and id is distinct from p_cut_id;

  if v_other_cuts_weight + p_weight > v_total_weight then
    raise exception 'Los cortes suman % kg, más que el peso de la res (% kg)', round(v_other_cuts_weight + p_weight, 3), v_total_weight;
  end if;
