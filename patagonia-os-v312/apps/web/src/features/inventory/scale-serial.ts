...
export interface PluNeighborEntry {
  argument: string;
  responseCode: string | null;
  internalPluNumber: string;
  name: string;
  price: string;
}

/** Diagnóstico puntual: lee con 5005 el PLU pedido y sus dos vecinos
 * (código-1, código, código+1), y muestra de cada uno el "Número de PLU"
 * interno (primeros 6 caracteres del DATA) y el nombre (siguientes,
 * después de depto+familia) -- para ver exactamente qué hay guardado
 * alrededor de un código puntual sin asumir nada sobre dónde aterrizó un
 * envío. No escribe nada, solo lee tres veces. */
export async function inspectPluNeighborhood(code: string): Promise<PluNeighborEntry[]> {
  if (!isScaleSerialSupported()) {
    throw new Error("Este navegador no soporta comunicación serie directa (usá Chrome o Edge).");
  }
  const settings = getScaleSerialSettings();
  const port = await pickPort();
  const base = Number(code) || 0;
  const argsToTry = [base - 1, base, base + 1];

  const results: PluNeighborEntry[] = [];
  for (const arg of argsToTry) {
    const pluArg = fixedDigits(arg, 6);
    const frame = buildFrame(settings.deviceType, settings.equipmentId, "5005", pluArg);
    const response = await writeFrameResilient(port, frame, settings.baudRate);
    const dataBytes = response.length > 10 ? response.slice(8, -3) : new Uint8Array();
    const dataStr = Array.from(dataBytes).map((b) => (b >= 0x20 && b <= 0x7e ? String.fromCharCode(b) : ".")).join("");
    const entry: PluNeighborEntry = {
      argument: pluArg,
      responseCode: extractResponseCode(response),
      internalPluNumber: dataStr.slice(0, 6),
      name: dataStr.slice(12, 38).trim(),
      price: dataStr.slice(77, 83)
    };
    console.log(`5005 argumento "${pluArg}": código "${entry.responseCode}" -- PLU interno "${entry.internalPluNumber}" -- nombre "${entry.name}" -- precio "${entry.price}"`);
    results.push(entry);
...
export interface PreservePriceTestResult {
  before: { responseCode: string | null; fields: string[] };
  writeResponseCode: string | null;
  after: { responseCode: string | null; fields: string[] };
  priceUpdated: boolean;
}

/** LA prueba definitiva: lee el PLU tal cual está, cambia SOLO el campo
 * Precio (todos los demás campos -- flag de posición decimal, código de
 * etiqueta, campo 19, etc. -- se mandan de vuelta exactamente como se
 * leyeron, sin ningún valor fijo nuestro), y confirma si el precio nuevo
 * queda grabado. Si esto funciona y nuestro buildPluFrame normal no, el
 * problema es que los valores fijos que mandamos en los campos
 * secundarios rompen la modificación de registros que no creamos
 * nosotros. */
export async function runPreservePriceTest(pluCode: string, newPrice: number): Promise<PreservePriceTestResult> {
  if (!isScaleSerialSupported()) {
    throw new Error("Este navegador no soporta comunicación serie directa (usá Chrome o Edge).");
  }
  const settings = getScaleSerialSettings();
  const port = await pickPort();
  const readArgument = fixedDigits((Number(pluCode) || 0) - 1, 6);

  async function readOnce() {
    const frame = buildFrame(settings.deviceType, settings.equipmentId, "5005", readArgument);
    const response = await writeFrameResilient(port, frame, settings.baudRate);
    const dataBytes = response.length > 10 ? response.slice(8, -3) : new Uint8Array();
    const dataStr = Array.from(dataBytes).map((b) => String.fromCharCode(b)).join("");
    const fields = splitPluFields(dataStr);
    console.log(`Lectura PLU ${pluCode} (argumento ${readArgument}) -- código: ${extractResponseCode(response)}`);
    fields.forEach((v, i) => console.log(`  Campo ${i + 1} (ancho ${PLU_FIELD_WIDTHS[i]}): "${v}"`));
    return { responseCode: extractResponseCode(response), fields };
  }

  console.log("--- BEFORE ---");
  const before = await readOnce();
  if (before.responseCode !== "01" || before.fields.join("").trim() === "") {
    throw new Error(`No se pudo leer el PLU ${pluCode} (código "${before.responseCode}") -- no hay datos reales para preservar.`);
  }

  const fieldsForWrite = [...before.fields];
  fieldsForWrite[PRICE_FIELD_INDEX] = fixedDigits(newPrice, PLU_FIELD_WIDTHS[PRICE_FIELD_INDEX]);
  console.log(`--- ESCRITURA: mismos campos que BEFORE, precio -> "${fieldsForWrite[PRICE_FIELD_INDEX]}" ---`);

  const payload = fieldsForWrite.join("");
  if (payload.length !== 135) throw new Error(`Payload inválido: ${payload.length} caracteres en vez de 135.`);
  const frame = buildFrame(settings.deviceType, settings.equipmentId, settings.altaCommand, payload);
  const writeResponse = await writeFrameResilient(port, frame, settings.baudRate);
  const writeResponseCode = extractResponseCode(writeResponse);
  console.log(`Respuesta de la escritura -- código: "${writeResponseCode}"`);

  await new Promise((resolve) => setTimeout(resolve, 700));
  console.log("--- AFTER ---");
  const after = await readOnce();

  const priceUpdated = after.fields[PRICE_FIELD_INDEX] === fieldsForWrite[PRICE_FIELD_INDEX];
  console.log(priceUpdated ? "RESULTADO A: el precio SÍ se actualizó preservando los demás campos." : "RESULTADO B: el precio NO se actualizó ni preservando los demás campos.");

  return { before, writeResponseCode, after, priceUpdated };
}

/** Arma una trama de escritura (2005) mínima con un Campo 01/06 elegido a
 * mano (sin el +1 ni ninguna lógica de Product) -- solo para el diagnóstico
 * de runSyntheticPluTest, en una zona de PLUs sintética que no toca ningún
 * producto real. */
function buildSyntheticPluFrame(deviceType: string, equipmentId: string, altaCommand: string, pluDigits: number, name: string, price: number): Uint8Array {
  const pluNumber = fixedDigits(pluDigits, 6);
  const pluCode = fixedDigits(pluDigits, 5);
  const data =
    `${pluNumber}${fixedDigits(1, 3)}${fixedDigits(1, 3)}${fixedAscii(name, 26)}${fixedAscii(name, 26)}${pluCode}P${fixedDigits(0, 7)}` +
    `${fixedDigits(price, 6)}${fixedDigits(0, 6)}${fixedDigits(2, 6)}${fixedDigits(0, 6)}${fixedDigits(0, 6)}${fixedDigits(0, 5)}${fixedDigits(0, 5)}` +
    `${fixedDigits(0, 2)}${fixedDigits(0, 4)}${fixedDigits(0, 4)}${fixedDigits(0, 4)}${fixedDigits(0, 4)}`;
  if (data.length !== 135) throw new Error(`Payload sintético inválido: ${data.length} caracteres.`);
  return buildFrame(deviceType, equipmentId, altaCommand, data);
}

async function readPluNext(port: SerialPort, settings: ScaleSerialSettings, argument: number): Promise<{ responseCode: string | null; internalPluNumber: string; name: string; price: string }> {
  const frame = buildFrame(settings.deviceType, settings.equipmentId, "5005", fixedDigits(argument, 6));
  const response = await writeFrameResilient(port, frame, settings.baudRate);
  const dataBytes = response.length > 10 ? response.slice(8, -3) : new Uint8Array();
  const dataStr = Array.from(dataBytes).map((b) => (b >= 0x20 && b <= 0x7e ? String.fromCharCode(b) : ".")).join("");
  return { responseCode: extractResponseCode(response), internalPluNumber: dataStr.slice(0, 6), name: dataStr.slice(12, 38).trim(), price: dataStr.slice(77, 83) };
}

export interface SyntheticPluTestResult {
  zoneEmptyCheck: { responseCode: string | null; internalPluNumber: string; name: string };
  firstWriteResponseCode: string | null;
  afterFirstWrite: { responseCode: string | null; internalPluNumber: string; name: string; price: string };
  secondWriteResponseCode: string | null;
  afterSecondWrite: { responseCode: string | null; internalPluNumber: string; name: string; price: string };
  updatedInPlace: boolean;
}

/** Prueba de aislamiento total en una zona sintética (PLU 90001, lejos de
 * cualquier código real de Patagonia OS) para determinar si el comando
 * 2005 MODIFICA un registro existente con el mismo Campo 01, o si agrega
 * un registro nuevo cada vez (duplicando). No toca ningún producto real:
 * 1. Verifica que la zona esté vacía (lee "próximo después de 89999").
 * 2. Escribe PLU 90001 con nombre "TEST SINTETICO A".
 * 3. Relee y confirma dónde quedó.
 * 4. Vuelve a escribir el MISMO PLU 90001 con nombre "TEST SINTETICO B" y
 *    otro precio.
 * 5. Relee: si ahora aparece "B", modificó en el lugar (correcto). Si sigue
 *    apareciendo "A", agregó un duplicado y la búsqueda encontró el viejo
 *    primero -- confirmaría que 2005 no actualiza por Campo 01. */
export async function runSyntheticPluTest(): Promise<SyntheticPluTestResult> {
  if (!isScaleSerialSupported()) {
    throw new Error("Este navegador no soporta comunicación serie directa (usá Chrome o Edge).");
  }
  const settings = getScaleSerialSettings();
  const port = await pickPort();
  const TEST_PLU = 90001;

  console.log("--- Paso 1: verificar que la zona esté vacía (próximo después de 89999) ---");
  const zoneEmptyCheck = await readPluNext(port, settings, 89999);
  console.log(`  código "${zoneEmptyCheck.responseCode}" -- PLU interno "${zoneEmptyCheck.internalPluNumber}" -- nombre "${zoneEmptyCheck.name}"`);
  if (zoneEmptyCheck.internalPluNumber && Number(zoneEmptyCheck.internalPluNumber) > 0 && Number(zoneEmptyCheck.internalPluNumber) <= TEST_PLU) {
    throw new Error(`La zona NO está vacía -- ya existe un PLU interno "${zoneEmptyCheck.internalPluNumber}" ("${zoneEmptyCheck.name}") cerca de ${TEST_PLU}. Frenando para no arriesgar pisar un dato real.`);
  }

  console.log("--- Paso 2: escribir PLU 90001 = TEST SINTETICO A ---");
  const frameA = buildSyntheticPluFrame(settings.deviceType, settings.equipmentId, settings.altaCommand, TEST_PLU, "TEST SINTETICO A", 1111);
  const responseA = await writeFrameResilient(port, frameA, settings.baudRate);
  const firstWriteResponseCode = extractResponseCode(responseA);
  console.log(`  respuesta código "${firstWriteResponseCode}"`);

  await new Promise((resolve) => setTimeout(resolve, 700));
  console.log("--- Paso 3: releer (argumento 90000) ---");
  const afterFirstWrite = await readPluNext(port, settings, TEST_PLU - 1);
  console.log(`  código "${afterFirstWrite.responseCode}" -- PLU interno "${afterFirstWrite.internalPluNumber}" -- nombre "${afterFirstWrite.name}" -- precio "${afterFirstWrite.price}"`);

  console.log("--- Paso 4: volver a escribir el MISMO PLU 90001 = TEST SINTETICO B ---");
  const frameB = buildSyntheticPluFrame(settings.deviceType, settings.equipmentId, settings.altaCommand, TEST_PLU, "TEST SINTETICO B", 2222);
  const responseB = await writeFrameResilient(port, frameB, settings.baudRate);
  const secondWriteResponseCode = extractResponseCode(responseB);
  console.log(`  respuesta código "${secondWriteResponseCode}"`);

  await new Promise((resolve) => setTimeout(resolve, 700));
  console.log("--- Paso 5: releer de nuevo (argumento 90000) ---");
  const afterSecondWrite = await readPluNext(port, settings, TEST_PLU - 1);
  console.log(`  código "${afterSecondWrite.responseCode}" -- PLU interno "${afterSecondWrite.internalPluNumber}" -- nombre "${afterSecondWrite.name}" -- precio "${afterSecondWrite.price}"`);

  const updatedInPlace = afterSecondWrite.name === "TEST SINTETICO B";
  console.log(updatedInPlace ? "RESULTADO: 2005 modificó en el lugar (correcto)." : "RESULTADO: 2005 NO modificó en el lugar -- sigue viéndose el valor viejo o algo distinto.");

  return { zoneEmptyCheck, firstWriteResponseCode, afterFirstWrite, secondWriteResponseCode, afterSecondWrite, updatedInPlace };
}
