...
function mapDebt(row: CreditorDebtRow): CreditorDebt {
  return { id: row.id, creditorId: row.creditor_id, debtDate: row.debt_date, amount: Number(row.amount), reason: row.reason, createdAt: row.created_at };
}
...
export async function listCreditorDebts(creditorId: string): Promise<CreditorDebt[]> {
  ...
  const { data, error } = await supabase
    .from("creditor_debts")
    .select("id,creditor_id,debt_date,amount,reason,created_at")
